...
public class StickersPacksContentProvider extends ContentProvider {
    ...
    public static final Uri AUTHORITY_URI = new Uri.Builder().scheme(ContentResolver.SCHEME_CONTENT).authority(BuildConfig.CONTENT_PROVIDER_AUTHORITY).appendPath(StickersPacksContentProvider.METADATA).build();
    ...
    private AssetFileDescriptor fetchFile(@NonNull Uri uri, @NonNull AssetManager am, @NonNull String fileName, @NonNull String identifier) {
        try {
            ...
            Log.e(getContext().getPackageName(), "IOException when getting asset file, uri:" + uri, e);
            return null;
        }
    }
